...
# 184
for num in range(184,185):
    # 要请求的网址：B站视频网址
    # 这个变量需要替换成你要下载的视频网址
    current = str(num)

    url = "https://www.bilibili.com/video/BV1Dh41167W4?p=" + str(num) + "&vd_source=7538539a9a61c62cdc9ff354afc11811"
    print(url)
    ...
    title = re.findall('<ul class="tag-area clearfix"><li class="tag"><div title="(.*?)"', response.text)[0]
    # 如果标题里有[\/:*?<>|]特殊字符，直接删除
    ...
    duration = json_data["data"]["dash"]["duration"]
    # objStr = {}
    # objStr.id = (index + 1) + 'L'
    # objStr.name = item.split('.')[0]
    # objStr.duration = duration
    # objStr.singer = '无名氏'
    # objStr.album = '群星'
    # objStr.image = ''
    # objStr.url = 'http://127.0.0.1:8080/music/' + title

    # audio_url = json_data["data"]["durl"][0]["url"]
    ...
    cmd ="ffmpeg -i {title}.mp4 -i {title}.mp3 -c:v copy -c:a aac -strict experimental {title}(最终版).mp4"
